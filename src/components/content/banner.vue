<template>
    <!-- 没有实现触摸滑动 -->
    <div class="slider">
        <ul :class="['items', hasTransition?'addTransition':'removeTransition']" :style="containerStyle">
            <li><img :src="imgs[imgs.length-1]" alt=""></li>
        ...
        <ul class="points">
            <li v-for="(item, index) in imgs" :key="index" :class="{'active':index==currentIndex}" @click="clickPoints(index)"></li>
        </ul>
    </div>
</template>
<script>
export default {
    ...
    watch: {
        distance() {
            ...
            if (_this.distance == -_this.$store.state.windowWidth * (_this.imgs.length + 1)) {
                setTimeout(() => {
    ...
    .slider {
        ...
        .btns {
            li {
                ...
                position: absolute;
                top: calc(50% - 15px);  // 注：减号前后要有空格，否则很可能不生效！
                &:first-child {
                    left: 20px;
                    &::before {
                        content: '';
